...
case "$setting" in
  "iterations")
    send_broadcast "set_iterations" "$value"
    ;;
  "min_interval")
    send_broadcast "set_min_interval" "$value"
    ;;
  "max_interval")
    send_broadcast "set_max_interval" "$value"
    ;;
  "url")
    # For URLs, use --es instead of --ei
    echo "Setting set_url to $value"
    adb shell "am broadcast -a $BROADCAST_ACTION --es command set_url --es value \"$value\" -p $PACKAGE"
    ;;
  "delay")
    send_broadcast "set_airplane_delay" "$value"
    ;;
  *)
