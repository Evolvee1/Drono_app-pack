 ...
 * 
 * Start simulation:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command start
 * 
 * Pause simulation:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command pause
 * 
 * Resume simulation:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command resume
 * 
 * Stop simulation:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command stop
 * 
 * Set URL:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command set_url --es value "https://example.com"
 * 
 * Set iterations:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command set_iterations --ei value 100
 * 
 * Set min interval:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command set_min_interval --ei value 5
 * 
 * Set max interval:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command set_max_interval --ei value 20
 * 
 * Set airplane mode delay:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command set_airplane_delay --ei value 3000
 * 
 * Enable/disable feature:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command toggle_feature --es feature "rotate_ip" --ez value true
 * 
 * Export configuration:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command export_config --es name "my_config" --es desc "My configuration description"
 * 
 * Import configuration:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command import_config --es name "my_config.json"
 * 
 * Get status:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command get_status
 * 
 * List available configurations:
 * adb shell am broadcast -a com.example.imtbf2.debug.COMMAND --es command list_configs
 */
public class AdbCommandReceiver extends BroadcastReceiver {
    private static final String TAG = "AdbCommandReceiver";
    public static final String ACTION_COMMAND = "com.example.imtbf2.debug.COMMAND";
    public static final String ACTION_RESPONSE = "com.example.imtbf2.debug.RESPONSE";
        ...
        switch (command) {
            ...
            case COMMAND_EXPORT_CONFIG:
                ...
                if (configName != null && !configName.isEmpty()) {
                    ...
                    if (configFile != null) {
                        sendResponse(context, true, "Configuration exported to: " + configFile.getName());
                    } else {
            ...
            case COMMAND_IMPORT_CONFIG:
                ...
                if (fileName != null && !fileName.isEmpty()) {
                    ...
                    for (File file : configManager.getSavedConfigurations()) {
    ...
    private void refreshMainActivityUI(Context context) {
        try {
            ...
            Intent refreshIntent = new Intent("com.example.imtbf2.REFRESH_UI");
            ...
            directIntent.setAction("com.example.imtbf2.REFRESH_UI");
