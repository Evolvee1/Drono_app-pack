...
ACTION="com.example.imtbf.debug.COMMAND"

# Function to send broadcast command
send_broadcast() {
  echo "Sending broadcast command: $1"
  adb shell am broadcast -a $ACTION -e command "$1" -p $PACKAGE
  sleep 1
}
...
send_broadcast_with_params() {
  ...
  adb shell am broadcast -a $ACTION -e command "$command" $@ -p $PACKAGE
  sleep 1
}
...
case "$1" in
  ...
  "set_url")
    ...
    send_broadcast_with_params "set_url" "-e value '$2'"
    ;;
  "set_iterations")
    if [ -z "$2" ]; then
    ...
    send_broadcast_with_params "set_iterations" "-e value $2"
    ;;
  "set_min_interval")
    if [ -z "$2" ]; then
      echo "Error: Number required"
    ...
    send_broadcast_with_params "set_min_interval" "-e value $2"
    ;;
  "set_max_interval")
    if [ -z "$2" ]; then
      echo "Error: Number required"
    ...
    send_broadcast_with_params "set_max_interval" "-e value $2"
    ;;
  "set_delay")
    if [ -z "$2" ]; then
      echo "Error: Number required"
    ...
    send_broadcast_with_params "set_airplane_delay" "-e value $2"
    ;;
  "toggle")
    if [ -z "$2" ]; then
    ...
    send_broadcast_with_params "toggle_feature" "-e feature $2 -e value $3"
    ;;
  "list_configs")
